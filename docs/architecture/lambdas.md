...
- DB access: RDS Proxy with IAM auth (`activities_app`) when
  ...
  `fixtures/activity_search_staging.json` bundled with the Lambda (see
  `shared/fixtures/activity_search_staging.json`, generated by
  `scripts/codegen/generate_activity_search_staging.py`).
- Environment:
