...
- DB access: RDS Proxy with IAM auth (`activities_app`) when
  `STAGING_SEARCH_DATA_ENABLED` is not `true`; otherwise reads
  `fixtures/activity_search_staging.json` copied into the Lambda bundle at
  deploy from `shared/fixtures/activity_search_staging.json` (Git LFS;
  generated by `scripts/codegen/generate_activity_search_staging.py`).
- Environment:
  - `STAGING_SEARCH_DATA_ENABLED` (`false` unless GitHub variable
    `STAGING_SEARCH_DATA_ENABLED` is `true` for the deploy — passed as CDK
    context `stagingSearchDataEnabled` in `deploy-backend.yml`)
  - `STAGING_SEARCH_DATA_PATH` (default `/var/task/fixtures/activity_search_staging.json`)
