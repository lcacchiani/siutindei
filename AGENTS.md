...
- Staging activity search fixture: generate with `python3 scripts/codegen/generate_activity_search_staging.py`, then `bash scripts/codegen/sync-activity-search-staging-fixture.sh` (syncs to `backend/fixtures/`, `apps/public_www/src/data/`, and `apps/siutindei_app/assets/fixtures/`). Enable with `STAGING_SEARCH_DATA_ENABLED` (Lambda) or `NEXT_PUBLIC_STAGING_SEARCH_DATA_ENABLED` / `STAGING_SEARCH_DATA_ENABLED` dart-define (Flutter).
