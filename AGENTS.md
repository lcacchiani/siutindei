...
- Staging activity search fixture: canonical file is `shared/fixtures/activity_search_staging.json` (Git LFS). Generate with `python3 scripts/codegen/generate_activity_search_staging.py`, then `bash scripts/codegen/sync-activity-search-staging-fixture.sh` (copies to `backend/fixtures/` for Lambda and `apps/public_www/public/fixtures/` for static CDN). After clone run `git lfs pull`. Public www and Flutter load the JSON at runtime from `/fixtures/activity_search_staging.json` (or `NEXT_PUBLIC_STAGING_SEARCH_FIXTURE_URL` / `STAGING_SEARCH_FIXTURE_URL`). Lambda uses `STAGING_SEARCH_DATA_ENABLED`.
